...
use l2_loop_core::{
    AlertCode, AlertSeverity, DetectionState, EVIDENCE_LIST_DEFAULT_LIMIT, EVIDENCE_LIST_MAX_LIMIT,
    ...
    EvidenceCursor, EvidenceListQuery, EvidenceStatus, INCIDENT_OUTPUT_QUEUE_CAPACITY,
    InterfaceName,
};
...
fn event_id_is_exact_canonical_lower_hex_and_never_path_capable() {
    ...
    assert_eq!(
        serde_json::from_str::<EventId>(&format!("\"{text}\"")).unwrap(),
        id
    );
...
fn passive_states_have_fixed_alert_severity_without_confirmed_loop() {
    let cases = [
        ...
        (
            ...
            AlertCode::ExternalLoopSuspected,
            AlertSeverity::Notice,
        ),
        (
...
fn evidence_list_query_enforces_limits_and_cursor_filter_binding() {
    ...
    assert_eq!(
        EvidenceCursor::parse_for(&encoded, Some(&interface)).unwrap(),
        cursor
    );
...
fn public_contract_source_has_no_raw_identity_or_active_action() {
    ...
    for prohibited in [
        ...
        assert!(
            ...
            "prohibited evidence API: {prohibited}"
        );
    }
}
