...
Assert-True ($Harness.Contains('install -m 0755 "$bundle/l2-loop-install" "$staging/usr/libexec/l2-loop/l2-loop-install"')) 'staging layout omits the installer'
Assert-True ($Harness.Contains('cleanup_file "$staging/usr/libexec/l2-loop/l2-loop-install"')) 'staging cleanup omits the installer'
Assert-True ($Harness.Contains('chmod 0755 l2-loopd l2-loopctl l2-loop-deploycheck l2-loop-hostcheck l2-loop-install')) 'remote installer mode is not executable-only'
