...
当前已实现的隔离被动指纹切片采用固定契约：对“2 字节大端帧长度 + 最多前 64 字节”计算 64 位 FNV-1a，低 4 位为零时采样（约 1/16），存入按 generation 隔离的 8,192 项 LRU。方向不参与哈希，因此未变化帧可以在 ingress/egress 之间关联。用户态只在 `observe/status` 请求中读取，经 journal 名称、固定路径和内核 Map ID 校验后输出有界关系计数；不公开原始指纹、MAC、报文内容、原始 key 或时间戳。该结果是可能受采样和 LRU 淘汰影响的证据下限，不是因果结论或环路判定。
