...
The currently implemented observation slice is deliberately narrow: it provides fail-open XDP ingress and TC egress cumulative observation, bounded rate windows, generation-scoped dynamic baselines, bounded passive frame fingerprints, privacy-reduced ingress/egress relationship evidence, and separate observation-health reporting for one generated, isolated namespace/veth session. It does not classify a storm or loop, send probes, apply policies, drop traffic, or attach to production, physical, bond, bridge, OVS, tap, or shared interfaces.

The broader product design remains observe-first. Later deliveries may add an observation-only loop-state machine, NIC/kernel and topology correlation, explicitly authorized one-frame probes, and expiring manual policing only after their own design and safety gates.
...
## Isolated observation, bounded rates, dynamic baseline, and relationships
...
Both commands perform a fresh request-time read of generation-scoped cumulative counters and the bounded fingerprint LRU. Client reads never insert rate or baseline samples and never recompute the cached baseline; the one-second background sampler never enumerates fingerprints. `observe` returns aggregate packets/bytes, six fixed mutually exclusive classes, parse-error counters, VLAN visibility, sampling diagnostics, detailed rates, the complete fixed-order baseline report, and a privacy-reduced fingerprint relationship report for XDP ingress and TC egress. `status` returns zero or one active session with cumulative hook aggregates, summarized hook rates, baseline state, sample counts, elevated identifiers, and the same privacy-reduced relationship summary.
...
- deterministic, privacy-reduced ingress/egress relationship reports in observation schema 4;
- a bounded host harness covering eleven exact-artifact regression, baseline, and fingerprint-lifecycle scenarios.
...
this delivery derives rates, baseline-relative evidence, and sampled ingress/egress relationships but never emits storm/loop verdicts, sends probes, drops traffic, or applies policies. It has no 100 ms sampler, persistent history, raw fingerprint output, or production-interface enablement.
